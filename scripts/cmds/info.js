...
module.exports = {
  ...
  onStart: async function ({ message, api, event }) {
    ...
    const botName = "Kate";
    const botPrefix = ">";
    const authorName = "Denver Crain";
    const authorFB = "https://www.facebook.com/profile.php?id=61551997800865";
    const authorInsta = "";
    const status = "Married 🙃🙃🙃";
    const timeStart = Date.now();

    const urls = JSON.parse(fs.readFileSync('anna.json'));
    const link = urls[Math.floor(Math.random() * urls.length)];

    // Get current date and time in Asia/Kathmandu timezone
    const now = moment().tz('Norway/Oslo');
