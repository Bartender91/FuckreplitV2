       ...
       const p = getPrefix(event.threadID);
		...
		if (fs.existsSync(pendingIDsPath)) {
			...
			if (pendingIDs.includes(threadID)) {
				const pendingMsg = "Your request to use bot main cmds is already in pending.... \nPlease contact Morgan Walker for fast approval.\n\n Facebook:https://www.facebook.com/morgan.kiplimo.35\n\n also you can join  Kate support Box for help\nType : ¶supportgroup \nto join.";
...
for (const adminID of config.adminBot) {
  api.sendMessage(msg, adminID);
		...
		const notifyMsg = `✅Your approval request for using Bot's main cmds has been sent to admin Morgan nd Denver with this\nmessage: ${userMessage}.\n\n I will notify you if your thread is approved. Please wait until then.\n\n join the Kate Fukari support Box if you facing any problem\nType : ¶support \nto join.`;
